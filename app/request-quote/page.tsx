...
import { inquiryAPI } from '../../src/services/api'
...
function RequestQuoteContent() {
  ...
  const [formData, setFormData] = useState({
    ...
    budget: '',
    timeline: '',
    description: productName ? `I am interested in: ${productName}. ` : '',
  ...
  const handleCheckboxChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData(prev => {
      const field = name as 'productCategories' | 'additionalServices'
      if (checked) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await inquiryAPI.submit(submissionData)
      alert('Your quote request has been submitted successfully! We will get back to you within 24 hours.')
      router.push('/')
    } catch (error: any) {
      console.error('Quote submission error:', error)
      alert(error.message || 'Failed to submit quote request. Please try again or contact us directly.')
    } finally {
      setLoading(false)
    }
  }
          ...
          <div className="lg:col-span-2">
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <button
                type="submit"
                disabled={loading}
                className={`w-full bg-hero-bg text-white py-4 px-6 rounded-lg font-semibold text-lg hover:bg-hero-bg-dark transition-colors duration-200 transform hover:-translate-y-0.5 hover:shadow-lg ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}
              >
