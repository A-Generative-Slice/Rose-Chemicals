...
class EmailService {
  ...
  async sendWelcomeEmail(user) {
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <a href="${process.env.NEXT_PUBLIC_FRONTEND_URL || 'https://rosechemicals.in'}/products" 
  ...
  async sendOrderConfirmation(user, order) {
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <a href="${process.env.NEXT_PUBLIC_FRONTEND_URL || 'https://rosechemicals.in'}/orders/${order._id}" 
           style="background-color: #e11d48; color: white; padding: 12px 24px; text-decoration: none; border-radius: 5px; display: inline-block;">
          Track Order
        </a>
  ...
  async sendPasswordReset(user, resetToken) {
    ...
    const frontendUrl = process.env.NEXT_PUBLIC_FRONTEND_URL || 'https://rosechemicals.in';
  ...
  async sendDeliveryOTP(user, order, otp) {
    const subject = `Your Delivery OTP - Order #${order._id.toString().slice(-8).toUpperCase()}`;
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h1 style="color: #e11d48;">Delivery OTP</h1>
        <p>Dear ${user.name},</p>
        <p>Your order #${order._id.toString().slice(-8).toUpperCase()} is out for delivery!</p>
        <p>Please share the following OTP with the delivery person only after you have received your package:</p>
        
        <div style="background-color: #f3f4f6; padding: 20px; text-align: center; font-size: 32px; font-weight: bold; letter-spacing: 5px; color: #e11d48; margin: 20px 0; border-radius: 8px;">
          ${otp}
        </div>
        
        <p>Once verified, your order status will be updated to "Delivered" and you can share your feedback on our website.</p>
