...
export default function ProductCard({ product }: { product: any }) {
  ...
  const handleAddToCart = async () => {
    if (!isAuthenticated) {
      router.push(`/auth/login?returnUrl=/products/${product._id || product.id}`);
      return;
    }
  ...
  const handleRequestQuote = () => {
    // Navigate to quote page with product info
    ...
    <div className="bg-tile-bg rounded-lg p-4 shadow-sm hover:shadow-md transition-all duration-200 border border-tertiary">
      <Link href={`/products/${product._id || product.id}`}>
        <div className="overflow-hidden rounded-lg bg-white cursor-pointer">
          <img
            src={getProductImageUrl(product.images?.[0]?.url) || product.image || '/images/placeholder-product.svg'}
