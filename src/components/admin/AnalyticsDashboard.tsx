        ...
        <div className="h-64 flex items-end gap-2">
          {salesData.map((data, index) => {
            ...
            return (
              ...
              >
                {/* Tooltip */}
                <div className="absolute bottom-full mb-2 hidden group-hover:block bg-gray-900 text-white text-xs rounded p-2 z-10 whitespace-nowrap shadow-lg">
                  <div className="font-semibold">{new Date(data.date).toLocaleDateString()}</div>
                  <div className="text-blue-200">Revenue: {formatCurrency(data.revenue)}</div>
                  <div className="text-green-200">Orders: {data.orders}</div>
                </div>

                <div className="flex items-end gap-1 w-full justify-center h-[85%]">
                  <div
                    className="bg-blue-600 rounded-t w-full max-w-[20px] min-h-[4px] transition-all duration-500 ease-out hover:bg-blue-500"
                    style={{ height: `${revenueHeight}%` }}
                  />
                  <div
                    className="bg-green-600 rounded-t w-full max-w-[20px] min-h-[4px] transition-all duration-500 ease-out hover:bg-green-500"
                    style={{ height: `${ordersHeight}%` }}
                  />
                </div>

                <div className="text-[10px] text-gray-500 w-full text-center truncate mt-2 h-[15%]">
